...
Technical configurations that are not to be transferred via startup parameters can also be defined via the file [`appsettings.json`](https://github.com/DIPFtba/IRTlibDeploymentSoftware/blob/main/docker/TestApp.Player/appsettings.json), which is contained in the directory `TestApp.Player`.
