---
title: "IRTlib Editor: Routing innerhalb von Erhebungsteilen"
format:
  html:
...
-   Grundlagen: <a href="#anchor-blockly-basics">Nutzung des *Blockly*-Editor</a>
    -   Anzeigen von <a href="#anchor-show-item">Items</a> oder <a href="#anchor-show-items">Itemlisten</a>
...
-   <a href="#anchor-adaptive-testdesigns">Adaptive Testdesigns</a> in *Blockly* und Verwendung von <a href="#anchor-itempools-and-IRT">ItemPools und IRT</a>
...
<font color="red">@SD / TODO: Ich hab hier erstmal eine vollständige Dokumentation versucht, die Testautoren benötigen um innerhalb von Erhebungsteilen das Routing zu definieren. Teile der Dokumentation wären aber identisch auch für das die Dokumentation von *Blockly* an anderen Stellen notwendig. Können wir doch auch Links ZWISCHEN Dokumenten in der Hilfe irgendwie ermöglichen?</font>
...
<div id="anchor-example-blockly-linear-sequence-01"/>
...
![](img/screenshot-show-items-from-list-lds2003-01-example-DEU){width="500"}
...
![](img/screenshot-show-items-lds2003-01-example-simple-DEU){width="280"}

<div id="anchor-example-blockly-simple-sequence-01"/>
...
<div id="anchor-example-blockly-time-restricted-01"/>
...
<div id="anchor-example-blockly-booklet-design-01"/>
...
![](img/screenshot-show-items-example-booklet-design-example-functions-01-DEU.png){width="600"}
...
![](img/screenshot-show-items-example-booklet-design-example-lists-01-DEU.png){width="600"}
...
<div id="anchor-general-notes"/>

## Allgemeine Hinweise zur Bearbeitung von Ablaufdefinitionen mit *Blockly*
...
<div id="anchor-delete"/>

### Löschen

Zum Löschen von Operatoren können diese auf den Papierkorb gezogen werden. Ausgewählte *Blockly*-Elemente können auch über die Taste *Entf(ernen)* gelöscht werden. Alternativ können ausgewählte *Blockly*-Elemente auch über Kontextmenü gelöscht werden.

### Redo-/Undo

Innerhalb des *Blockly*-Editor können einzelne Aktionen rückgängig gemacht werden. Dafür kann die Tastenkombination `Strg + Z` verwendet werden. Mit `Strg + Y` wird eine Aktion wiederholt. Durch einen Klick in einen leeren Bereich des *Blockly*-Editors ist der Zugriff auf ein Kontextmenü möglich, welches ebenfalls die Optionen für *Rückgängig* und *Wiederholen* bereithält:

![](img/screenshot-context-menu-blockly-editor-example01-DEU.png){width="200"}

### Speichern

Anpassungen im *Blockly*-Editor müssen gespeichert werden. Dafür steht unten rechts das Disketten-Symbol zur Verfügung:
...
Die Ansicht im Arbeitsbereich kann mit den Icons `+` vergrößert und mit `-` verkleinert werden.

<div id="anchor-context-menu"/>

### Kontextmenü

Weitere Optionen sind über die rechte Maustaste (Kontextmenü) im *Blocky*-Editor verfügbar. Um diese Funktionen aufzurufen, muss auf ein *Blockly*-Element ein Sekundärklick (rechte Maustaste) durchgeführt werden:
...
<font color="red">@SD / TODO: (Wie) Könnte der Scope auch aus einer Blockly-Variablen kommen?</font>

<div id="anchor-show-items"/>

### Anzeigen von Itemlisten
...
<font color="red">@SD / TODO: Datentypen String für einzelne und Array für mehrere Items? Warum geht "Show Item" nicht direkt mit String?</font>

<div id="anchor-back-navigation"/>

#### Zurücknavigation

Das *Show Items*-Element für <a href="anchor-lists">Listen</a> kann über die Eigenschaft *Can navigate back* konfiguriert werden. Ist diese Eigenschaft ausgewählt, dann können *CBA ItemBuilder Tasks* mit dem *Command* `BACK_TASK` eine Navigation zum vorherigen *CBA ItemBuilder Tasks* anfordern.

#### Abbrechen von Listen

Die Verwendung von Listen erlaubt auch das Abbrechen von Listen. Listen können über zwei Wege abgebrochen werden:
...
<font color="red">@SD / TODO: Hier fehlt mir noch die Definition / Beschreibung, wie die verschiedenen Optionen funktionieren.</font>
...
<font color="red">@SD / TODO: Könnte man für die Zeit auch eine Blockly-Variable über einen Eingang verbinden lassen? Dann würden sich sinnvolle Anwendungsfälle ergeben, in denen man die Zeit in einem nicht Zeitbeschränkten Abschnitt misst, und dann nur, wenn noch genug Zeit vorhanden ist einen weiteren Zeitbeschränkten Abschnitt betreten, wo man die verbleibende Zeit dann als Timer definieren könnte.</font>

<div id="anchor-test-administrator-menu"/>
...
#### Anpassen von Standardfunktionen

Folgende Standardfunktionen können für eine Studie im Abschnitt *Info* / *Testleitermenü* definiert werden:
...
#### Verwenden von *Blockly*-Funktionen im Testleitermenü

Der *Blockly*-Operator für das Bearbeiten des Testleitermenüs enthält im Abschnitt *Funktion* auch die Option zum Ausführen von *Blockly*-Code (*ExecuteBockly*):
...
<font color="red">@SD / TODO: Hier würde mir für ein Anwendungsbespiel jetzt die Möglichkeit fehlen, auf die Standardfunktionen im Ablauf über einen Blockly-Operator zuzugreifen. Wie ist das gedacht? Vor allem benötigt wird: "Aufgabe vor". Dann könnte man als Blockly-Funktion definieren, dass das aktuelle Item beendet und dann das Anzeigen weitere Items zu unterbinden, z.B. über die Zuweisung eines Werts zu einer (globalen) Variable.</font>
...
<div id="anchor-advanced-use"/>

## Fortgeschrittene *Blockly*-Verwendung

<div id="anchor-conditions"/>
...
<div id="anchor-logical-expressions"/>
...
#### Kombination von *logischen Ausdrücken*

Einzelne Bedingungen oder logische Ausdrücke können mit folgendem *Blockly*-Element aus dem Abschnitt *Logik* verbunden werden:
...
#### Mehrere Bedingungen (*sonst falls* / *sonst*)

Durch Klick auf das kleine Zahnrad-Symbol eines Bedingungsblocks (*falls/mache*) kann dieser konfiguriert werden:
...
### Spezialfall: *prüfe*-Operator für drei Bedingungen

Für drei Bedingungen stellt der *Blockly*-Editor eine speziellen Operator *prüfe-falls wahr-falls falsch* zur Verfügung:
...
![](img/screenshot-blockly-element-if-iftrue-iffalse-example-02-DEU.png){width="600"}

<font color="red">@SD / TODO: Habe ich das richtig interpretiert?</font>

#### Negation

Um einen logischen Ausdruck umzukehren (Negation) steht folgender *Blockly*-Operator zur Verfügung:
...
Der folgende *Blockly*-Operator kann verwendet werden, um die Ausführung von Blöcke n-mal zu wiederholen:

![](img/screenshot-blockly-loop-repeat-n-times-01-DEU.png){width="180"}

#### Wiederhole solange

Schleifen können auch solange wiederholt werden *bis* eine Bedingung zutrifft (oder *solange* eine Bedingung zutrift):
...
Schleife mit Hilfsvariablen:

![](img/screenshot-blockly-loop-for-01-DEU.png){width="350"}

<div id="anchor-loop-over-list"/>

#### Für jeden Wert aus Liste

Schleife über alle Werte einer Liste

![](img/screenshot-blockly-loop-list-01-DEU.png){width="250"}

#### Schleifen vorzeitig abbrechen

Folgendes *Blockly*-Element kann genutzt werden, um eine Schleife (vorzeitig) abzubrechen oder um vorzeitig mit dem nächsten Schleifendurchlauf zu beginnen:
...
<font color="red">@SD / TODO: Was ist "-"?</font>

#### Erzeugung von Zufallszahlen

Für die Erstellung von Zufallszahlen stehen zwei *Blocky*-Elemente zur Verfügung:
...
#### Numerische Funktionen für <a href="#anchor-lists">Listen</a>

Vordefinierte Funktionen für <a href="#anchor-lists">Listen</a> umfassen:
...
<font color="red">@SD / TODO: Was passiert, wenn man die Funktionen auf Listen mit "Tasks" anwendet?</font>
...
Zum erstellen von Text steht folgender Operator zur Verfügung:

![](img/screenshot-blockly-text-string-01-ENG.png){width="150"}

#### Verketten

Verschiedene Operatoren können verwendet werden, um Text zusammenzufügen und zu Variablen zuzuweisen:
...
Die Länge einer Zeichenkette kann mit folgendem *Blockly*-Operator ermittelt werden:

![](img/screenshot-blockly-text-length-operator-empty-01-DEU.png){width="150"}

#### Prüfung auf leeren String

Leere String-Variablen können daran erkannt werden, dass die Anzahl der Zeichen $0$ ist.
...
#### Position in String finden

Ein Operator, der *im Text* (der per Variable oder als Ausdruck übergeben wird) das *erste* oder *letzte* *Auftreten eines Begriffs* sucht, kann wie folgt verwendet werden:
...
<font color="red">@SD / TODO: Richtig?</font>

#### Teilzeichenketten bilden

Der folgende Operator nimmt aus der übergebenen Zeichenkette *im Text* die ersten Buchstaben. Die Anzahl der Buchstaben wird dabei ebenfalls übergeben.
...
<font color="red">@SD / TODO: Was ist der Unterschied zwischen 'nimm' und 'nimm ersten' bzw. 'nimm von hinten' und 'nimm letzten'?</font>
...
<font color="red">@SD / TODO: Was ist 'nimm Teil ab erster'?</font>

#### Texts Verändern

Vorhandene Texte (entweder als Ausdrücke oder aus Variablen vom <a href="#anchor-variables-datatype">*Datentyp*</a> *String*) können durch die Anwendung von Operatoren verändert werden.
...
<font color="red">@SD / TODO: Was macht die Option 'Substantive'?</font>
...
#### Festhalten von Zeitpunkten

Variablen vom <a href="#anchor-variables-datatype">*Datentyp*</a> *DateTime* können Zeitstempel zugewiesen werden:

<font color="red">@SD / TODO: </font>

#### Ermitteln von Zeitdifferenzen

Vollständiges Beispiel: Folgender *Blockly*-Code misst die Zeit für die Bearbeitung von Task 1 bis 4. Dafür wird zunächst der Startzeitpunkt fegesthalten, und nach der Bearbeitung der Aufgaben wird die Zeitdifferenz ermittelt und in Sekunden umgewandelt:
...
<font color="red">@SD / TODO: Der "Total Sceonds" Operator hat früher (d.h. auf https://blockly.software-driven.de/) erlaubt, die Einheit mit anzugeben (also Milliseconds, Seconds, Minutes etc., vgl. Bild was von der alten Implementierung stammt). Gab es einen bestimmten Grund das zu entfernen oder kommt das noch?</font>
...
Liste können aus besehenden Elementen erstellt werden:
...
<font color="red">@SD / TODO: Aus welchen Elementen kann man Listen erstellen?</font>

#### Operatoren für Listen

Verbinden von Listen:

Prüfung, ob eine Liste leer ist:

Auswahl der distinkten Elemente einer Liste:

#### Listen Sortieren

<font color="red">@SD / TODO: Hier wäre vielleicht noch eine Option sinnvoll, Listen zufällig zu sortieren (als zu Mischen). Dann könnte man, in den Fällen wo man Items in einer beliebigen Reihenfolge braucht das direkt umsetzen.</font>
...
<div id="anchor-variables"/>

### *Blockly*-Variablen

Der Abschnitt *Variables* der *Palette* enthält *Blockly*-Elemente zur Erstellung und Verwendung von Variablen. Variablen können über die Schaltfläche *Typisierte Variable erstellen* angelegt werden:

![](img/screenshot-variables-palette-create-variable-01-DEU.png){width="300"}

*Blockly*-Variablen haben immer einen *Variablennamen* und einen <a href="#anchor-variables-datatype">*Datentyp*</a>:

![](img/screenshot-variables-palette-create-variable-02-DEU.png){width="300"}

<font color="red">@SD / TODO: Wie kann man auf Parameter zugreifen im Blockly? Einfach eine Variable anlegen, die einem Parameter entspricht? Oder sollen die Parameter künftig automatisch im Blockly-Editor angelegt werden? </font>

<div id="anchor-variables-datatype"/>

#### Einfache Datentypen und Wertzuweisungen

Folgende basale Datentypen werden unterstützt:
...
<font color="red">@SD / TODO: Für Zeitspannen hätte ich gern auch Millisekunden, ggf. könnte man dann hier auch noch einen Setter mit MS haben wollen?</font>

-   *TimeSpan*: Zeitspanne

![](img/screenshot-blockly-assignment-timespan-01-DEU.png){width="200"}

<font color="red">@SD / TODO: Für Zeitspannen weiß ich nicht, wie ich den Wert zuweisen soll (außer über die Differenz von zwei DateTimes.</font>

#### Datentypen für mehrere Werte

Datentypen für mehrere Elemente:
...
<font color="red">@SD / TODO: Hier braucht es noch ein Beispiel / mehr Erklärung.</font>

-   *KeyTypedValuePairs*:

<font color="red">@SD / TODO: Hier braucht es noch ein Beispiel / mehr Erklärung.</font>

<font color="red">@SD / TODO: Welche weitere Erklärung braucht es für ItemDefinition, SessionEvent, ItemScore, Item(s)... ?</font>

#### Variablenwerte Verwenden

Für die Verwendung von Variablenwerten, können *Blockly*-Elemente mit *Eingängen* folgende Komponente aufnehmen:
...
Es können zwei verschiede Formen von Funktionen definiert werden.
...
#### Definieren von Rückgabewerten von Funktionen
...
#### Definieren von Aufrufparametern von Funktionen

Funktionen können auch Parameter verwenden, die beim Aufruf der Funktion zu übergeben sind (*Aufrufparametern*). Die Defintion von Aufrufparametern ist nach einem Klick auf das kleine Zahnrad-Symbol eines Funktions-Blocks möglich:
...
<div id="anchor-write-data"/>

### *Blockly*-Operatoren zum schreiben von Daten

#### Log-Daten

Folgender Operator kann genutzt werden, um Informationen direkt in die Log-Daten zu speichern:

![](img/screenshot-write-result-log-element-with-example-01-ENG.png){width="500"}

<font color="red">@SD / TODO: Könnte hier noch ein Operator fehlen, um etwas mit einer bestimmten Struktur als Log-Event gespeichert zu werten (EventName/Typ, Event-spezifische Attribute)?</font>

#### Ergebnis-Daten

<font color="red">@SD / TODO: Könnte hier noch ein Operator fehlen, um etwas in die Ergebnisdaten zu speichern?</font>

#### Monitoring-Daten

<font color="red">@SD / TODO: Wie kann man die Monitoring-Daten füllen?</font>
...
<div id="anchor-presentation"/>

## Darstellung von *Blockly*-Code

<div id="anchor-folding"/>
...
<font color="red">@SD / TODO: Ist das so implementiert?</font>
...
<div id="anchor-internal-external"/>

### Internal / External

Einige *Blockly*-Elemente mit *Eingängen* (d.h. Stellen, an denen man weitere Blöcke verbinde kann) erlauben zwischen zwei Darstellungsformen zu wechseln.
...
<div id="anchor-tidy-up"/>

### Aufräumen

Im Kontextmenü des *Blockly*-Editors, welches durch Klick in einen leeren Bereich geöffnet werden kann, ist die Funktion *Bausteine aufräumen* enthalten:
...
<div id="anchor-adaptive-testdesigns"/>

## Umsetzung einfacher adaptiver Testdesigns in *Blockly*

### Bedingter Abbruch

Eine Liste von Tasks soll nur solange administriert werden, bis drei mal in Folge eine bestimmte Antwort gegeben wurde:

![](img/screenshot-blockly-conditional-skip-example-01-DEU.png){width="700"}

### Multi-Stage Testing (MST)

#### Definition von Stufen

Zusammenstellung von Stufen (*Stages*) aus CBA ItemBuilder *Tasks*:

-   Definition von <a href="#anchor-lists">Listen</a> für einzelnen Stufen, jeweils aus konkreten Tasks:

![](img/screenshot-mst-example-stage-definition-as-list-01-DEU.png){width="400"}

-   Verwendung von <a href="#anchor-functions">Funktionen</a> für einzelne Stufen, jeweils aus konkreten Tasks:

#### Antworten Sammeln

Das Administrieren der *Tasks* einer Stufe erfolgt dann passend zur Definition der Stages.

-   <a href="#anchor-lists">Listen</a> für einzelnen Stufen werden vorgelegt durch:

![](img/screenshot-mst-example-stage-definition-as-list-02-DEU.png){width="200"}

-   <a href="#anchor-functions">Funktionen</a> für einzelne Stufen werden vorgelegt durch:

#### Fähigkeit Ermitteln

Rohscore (z.B. Anzahl richtige Antworten):

-   ...

<font color="red">@SD / TODO: Wie kommt man an die Ergebnisse???</font>

IRT-basierte Fähigkeitsschätzung aus Antworten erfordert zusätzlich die Definition eines <a href="#anchor-itempools-and-IRT">ItemPools</a>.
...
<div id="anchor-itempools-and-IRT"/>

## Adaptives Testen mit IRT und ItemPools (Under Development)

### On-the-fly MST

### Computerized Adaptive Testing (CAT)

### Mehrdimensionales Adaptives Teste (MAT)
...
<div id="anchor-ai-use"/>

## Nutzung von API-Funktionen in *Blockly* (Under Development)

### Kodieren offener Antworten

<font color="red">@SD / TODO: Hier stelle ich zwei Operatoren vor der wie folgt funktionieren:

-   Modus
    -   Synchron (Es wird gewartet, das Ergebnis kann dann sofort verwendet werden)
    -   Asynchron (Es wird nicht gewartet, das Ergebnis wird - wenn vorhanden - nach dem nächsten ShowItem(s) berücksichtigt)
-   Name der zu verwendenden API
    -   je Item kann das eine andere sein
    -   die API muss im Editor/Player registriert sein
-   Input
    -   Name der API
    -   Blockly-Variable mit Ergebnistext
    -   Timeout (z.B. 5 Sec.)
