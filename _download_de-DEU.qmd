Die *IRTlib*-Software wird für die Offline-Nutzung (z.Zt. für Windows-Betriebssysteme) und für die Online-Nutzung (in Form von *Docker-Container*) bereitgestellt.
...
* **`TestApp.Editor.Desktop.exe`**: Ist in `TestApp.Player.Desktop.zip`  und muss gestartet werden, umd den  *IRTlib Editor* zu verwenden. 

* **`TestApp.Player.Desktop.exe`**: Ist in `TestApp.Editor.Desktop.zip` enthalten und muss gestartet werden, umd den  *IRTlib Player* zu verwenden. 
